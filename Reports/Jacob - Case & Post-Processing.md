...
# Analysis
This document presents an analysis of the post-processing and case subsystem. The post-processing section is designed to receive PWM and TACH signals as inputs from the microcontroller. These signals are then stepped up to the appropriate voltage levels. Specifically, the PWM signal is increased from 3.3V to 24V, while the TACH signal is elevated from 3.3V to a range of 10-12V. The PWM circuit outputs a current of 10mA, and the TACH circuit outputs 5mA.  

The circuit begins with the PWM and TACH signal currents flowing through a 210 Ohm resistor. This configuration ensures that the forward current passing through the opto-coupler's LED remains within the desired range, thereby enhancing the longevity of the component and providing an accurate signal for processing. The selected opto-coupler, a PC817A, is well-suited for the 6 kHz PWM signal that needs to be processed. It can handle a maximum voltage of up to 40 Vdc, as specified in the datasheet, and has a cutoff frequency of 80 kHz with rise and fall times of up to 18 µs. These specifications allow the component to process the 6 kHz frequency signal without introducing significant distortion or delay. The 210 Ohm resistor value was determined using the formula ((V1-Vf)/i), calculated as ((3.3V - 1.2V)/10ma). On the output side of the opto-coupler for the PWM signal, a 2k Ohm resistor is employed. This resistor was originally 1.2k Ohms, but testing revealed that a 2k Ohm resistor produced a better output signal.  

The TACH signal required a more complex approach because it needed to be stepped up specifically to 10V, as requested by Lochinvar. Similar to the PWM signal, a 2k Ohm step-up resistor was used to improve the signal quality. However, in this case, a 10V to 40V linear converter was added to achieve the desired TACH output range of 10-12V.  
...
These picture shows the PWM and TACH out signals. The PWM shows the 24V output and the TACH shows the how the linear converter needs about 15ms to hit peak output voltage. Then the current going to the opto-coupler will be the same for both. Also you can see in the TACH ouput that it ramps up over time due to the linear converter.
